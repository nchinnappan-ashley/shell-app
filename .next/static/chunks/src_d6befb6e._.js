...
const catalogAgent = {
    ...
    detect: (text)=>{
        ...
        const hit = keywords.some((k)=>t.includes(k)) ? 0.6 : 0;
        const matches = searchCatalog(t, 1);
        return Math.max(hit, matches.length ? 0.7 : 0);
    },
    slots: [
        {
            key: 'query',
...
function processUserInput(session, userText) {
    const replies = [];
    ...
    if (!session) {
        ...
        if (agent.id === 'catalog.search') {
            ...
            newSession.slotIndex = 1; // move to next
        ...
        replies.push(ask !== null && ask !== void 0 ? ask : 'Working on it…');
        return {
            replies,
            session: newSession
        };
    ...
    // Execute
    const out = agent.execute(session.slots);
    if (out instanceof Promise) {
        // If an agent ever becomes async, keep session and let UI poll/continue
...
function ChatDock() {
    ...
    const send = ()=>{
        ...
        setInput("");
        // Agentic processing with slot-filling
        const result = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$agents$2f$orchestrator$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["processUserInput"])(session, text);
...
_s1(ChatDock, "kx3UTYWP66RCUMUsvS4aQz1ItDY=", false, function() {
    return [
...
//# sourceMappingURL=src_d6befb6e._.js.map